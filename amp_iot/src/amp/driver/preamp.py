...
class Preamp:
    ...
    _addrLoudness       = 65  # 0b01000001 #
    _addrSoftMute       = 66  # 0b01000010 #
    _addrMainVolume     = 67  # 0b01000011 #
    _addrTreble         = 68  # 0b01000100 #
    _addrMiddle         = 69  # 0b01000101 #
    _addrBass           = 70  # 0b01000110 #
    _addrSubSetup       = 72  # 0b01001000 #
    _addrSecondSource   = 71  # 0b01000111 #
    _addrMixGain        = 73  # 0b01001001 #
    _addrLFSpeaker      = 74  # 0b01001010 #
    _addrRFSpeaker      = 75  # 0b01001011 #
    _addrLRSpeaker      = 76  # 0b01001100 #
    _addrRRSpeaker      = 77  # 0b01001101 #
    _addrMixLevel       = 78  # 0b01001110 #
    _addrSubVolume      = 79  # 0b01001111 #
    _addrDriverSetup    = 80  # 0b01010000 #
    ...
    def init(self):
        ...
        # data to write on device over I2C
        self._DataMainSource    = 0 #self.data.get(self._keyDataMainSource)
        self._DataSecondSource  = 0 #self.data.get(self._keyDataSecondSource)
        self._DataLoudness      = 0 #self.data.get(self._keyDataLoudness)
        self._DataSoftMute      = 0 #self.data.get(self._keyDataSoftMute)
        self._DataTreble        = 0 #self.data.get(self._keyDataTreble)
        self._DataMiddle        = 0 #self.data.get(self._keyDataMiddle)
        self._DataBass          = 0 #self.data.get(self._keyDataBass)
        self._DataSubSetup      = 0 #self.data.get(self._keyDataSubSetup)
        self._DataMixLevel      = 0 #self.data.get(self._keyDataMixLevel)
        self._DataDriverSetup   = 0

        # volumes data
        self._MainVolume        = 0 #self.data.get(self._keyMainVolume)
        self._LFSpeaker         = 0 #self.data.get(self._keyLFSpeaker)
        self._RFSpeaker         = 0 #self.data.get(self._keyRFSpeaker)
        self._LRSpeaker         = 0 #self.data.get(self._keyLRSpeaker)
        self._RRSpeaker         = 0 #self.data.get(self._keyRRSpeaker)
        self._SubVolume         = 0 #self.data.get(self._keySubVolume)
        self._MixGain           = 0 #self.data.get(self._keyMixGain)
    ...
    # set softStep for all volumes
    # also reinit volumes
    def setVolumesSoftStep(self, softStep):
        self._VolumesSoftStep = self.limit(softStep, 1)
        self._preamp_data.set(self._keyVolumesSoftStep, self._VolumesSoftStep).save()
        self._reinitVolumes()
        return self

    # get softStep (general soft step)
    def getVolumesSoftStep(self):
...
############## VOLUMES ##############
    ...
    def setMainVolume(self, value, softStep=None):
        if None == softStep:
            softStep = self._VolumesSoftStep

        self._MainVolume = self._setVolume(value, self._addrMainVolume, self._keyMainVolume, softStep)
        return self
    ...
    # value 0-96 +0, +1...+15 -0, -1, -2...-78, -79, mute dB
    def setLFSpeakerVolume(self, value, softStep=None):
        if None == softStep:
            softStep = self._VolumesSoftStep

        self._LFSpeaker = self._setVolume(value, self._addrLFSpeaker, self._keyLFSpeaker, softStep)
        return self

    def getLFSpeakerVolume(self):
        return self._LFSpeaker
    
    # set right front speaker volume
    # value 0-96 +0, +1...+15 -0, -1, -2...-78, -79, mute dB
    def setRFSpeakerVolume(self, value, softStep=None):
        if None == softStep:
            softStep = self._VolumesSoftStep

        self._RFSpeaker = self._setVolume(value, self._addrRFSpeaker, self._keyRFSpeaker, softStep)
        return self
    
    def getRFSpeakerVolume(self):
        return self._RFSpeaker
    
    # set left rear speaker volume
    # value 0-96 +0, +1...+15 -0, -1, -2...-78, -79, mute dB
    def setLRSpeakerVolume(self, value, softStep=None):
        if None == softStep:
            softStep = self._VolumesSoftStep

        self._LRSpeaker = self._setVolume(value, self._addrLRSpeaker, self._keyLRSpeaker, softStep)
        return self
    
    def getLRSpeakerVolume(self):
        return self._LRSpeaker
    
    # set right rear speaker volume
    # value 0-96 +0, +1...+15 -0, -1, -2...-78, -79, mute dB
    def setRRSpeakerVolume(self, value, softStep=None):
        if None == softStep:
            softStep = self._VolumesSoftStep

        self._RRSpeaker = self._setVolume(value, self._addrRRSpeaker, self._keyRRSpeaker, softStep)
        return self
    
    def getRRSpeakerVolume(self):
        return self._RRSpeaker
    
    # set right rear speaker volume
    # value 0-96 +0, +1...+15 -0, -1, -2...-78, -79, mute dB
    def setSubVolume(self, value, softStep=None):
        if None == softStep:
            softStep = self._VolumesSoftStep

        self._SubVolume = self._setVolume(value, self._addrSubVolume, self._keySubVolume, softStep)
        return self
    ...
    # value 0-96 +0, +1...+15 -0, -1, -2...-78, -79, mute dB
    def setMixGain(self, value, softStep=None):
        if None == softStep:
            softStep = self._VolumesSoftStep

        self._MixGain = self._setVolume(value, self._addrMixGain, self._keyMixGain, softStep)
        return self
    ...
    # set general volume
    def _setVolume(self, value, addr, saveKey=False, softStep=0):
        volume = self.limit(value, 96)
        data = self.setBits(self._volumeMap[volume], softStep, 1, 7)
    ...
    def setSecondSource(self, value, save=1):
        value = self.limit(value, 5)
        self._DataSecondSource = self.setBits(self._DataSecondSource, value, 7, 0)
        self.write(self._addrSecondSource, self._DataSecondSource)
    ...
    def setSecondSourceGain(self, value, save=1):
        value = self.limit(value, 15)
        self._DataSecondSource = self.setBits(self._DataSecondSource, value, 15, 3)
        self.write(self._addrSecondSource, self._DataSecondSource)
    ...
    def setSecondSourceRearSource(self, value, save=1):
        ...
        self._DataSecondSource = self.setBits(self._DataSecondSource, value, 1, 7)
        self.write(self._addrSecondSource, self._DataSecondSource)
    ...
    def setLoudness(self, value, save=1):
        value = self.limit(value, 15)
        self._DataLoudness = self.setBits(self._DataLoudness, value, 15, 0)
        self.write(self._addrLoudness, self._DataLoudness)
    ...
    def setLoudnessFreq(self, value, save=1):
        value = self.limit(value, 3)
        self._DataLoudness = self.setBits(self._DataLoudness, value, 3, 4)
        self.write(self._addrLoudness, self._DataLoudness)
    ...
    def setLoudnessBoost(self, value, save=1):
        value = self.limit(value, 1)
        self._DataLoudness = self.setBits(self._DataLoudness, value, 1, 6)
        self.write(self._addrLoudness, self._DataLoudness)
    ...
    def setLoudnessSoftStep(self, value, save=1):
        value = self.limit(value, 1)
        self._DataLoudness = self.setBits(self._DataLoudness, value, 1, 7)
        self.write(self._addrLoudness, self._DataLoudness)
    ...
    def setSoftMutePin(self, value, save=1):
        value = self.limit(value, 1)
        self._DataSoftMute = self.setBits(self._DataSoftMute, value, 1, 1)
        self.write(self._addrSoftMute, self._DataSoftMute)
    ...
    def setSoftMuteTime(self, value, save=1):
        value = self.limit(value, 2)
        self._DataSoftMute = self.setBits(self._DataSoftMute, value, 7, 2)
        self.write(self._addrSoftMute, self._DataSoftMute)
    ...
    def setSoftMuteTimeStep(self, value, save=1):
        value = self.limit(value, 7)
        self._DataSoftMute = self.setBits(self._DataSoftMute, value, 7, 4)
        self.write(self._addrSoftMute, self._DataSoftMute)
    ...
    def setTreble(self, value, save=1):
        value = self.limit(value, 15, -15)
        sign = 1
        if value < 0:
            sign = 0
        self._DataTreble = self.setBits(self._DataTreble, abs(value), 15, 0)
        self._DataTreble = self.setBits(self._DataTreble, sign, 1, 4)
        self.write(self._addrTreble, self._DataTreble)
    ...
    def setTrebleCenterFreq(self, value, save=1):
        value = self.limit(value, 3)
        self._DataTreble = self.setBits(self._DataTreble, value, 3, 5)
        self.write(self._addrTreble, self._DataTreble)
    ...
    def setTrebleReferenceOutput(self, value, save=1):
        value = self.limit(value, 1)
        self._DataTreble = self.setBits(self._DataTreble, value, 1, 7)
        self.write(self._addrTreble, self._DataTreble)
    ...
    def setMiddle(self, value, save=1):
        ...
        if value < 0:
            sign = 0
        self._DataMiddle = self.setBits(self._DataMiddle, abs(value), 15, 0)
        self._DataMiddle = self.setBits(self._DataMiddle, sign, 1, 4)
        self.write(self._addrMiddle, self._DataMiddle)
    ...
    def setMiddleQFactor(self, value, save=1):
        value = self.limit(value, 3)
        self._DataMiddle = self.setBits(self._DataMiddle, value, 3, 5)
        self.write(self._addrMiddle, self._DataMiddle)
    ...
    def setMiddleSoftStep(self, value, save=1):
        value = self.limit(value, 1)
        self._DataMiddle = self.setBits(self._DataMiddle, value, 1, 7)
        self.write(self._addrMiddle, self._DataMiddle)
    ...
    def setBass(self, value, save=1):
        ...
        if value < 0:
            sign = 0
        self._DataBass = self.setBits(self._DataBass, abs(value), 15, 0)
        self._DataBass = self.setBits(self._DataBass, sign, 1, 4)
        self.write(self._addrBass, self._DataBass)
    ...
    def setBassQFactor(self, value, save=1):
        value = self.limit(value, 3)
        self._DataBass = self.setBits(self._DataBass, value, 3, 5)
        self.write(self._addrBass, self._DataBass)
    ...
    def setBassSoftStep(self, value, save=1):
        value = self.limit(value, 0)
        self._DataBass = self.setBits(self._DataBass, value, 0, 0)
        self.write(self._addrBass, self._DataBass)
    ...
    def setSubSetupCutFreq(self, value, save=1):
        value = self.limit(value, 3)
        self._DataSubSetup = self.setBits(self._DataSubSetup, value, 3, 0)
        self.write(self._addrSubSetup, self._DataSubSetup)
    ...
    def setSubSetupMiddleFreq(self, value, save=1):
        value = self.limit(value, 3)
        self._DataSubSetup = self.setBits(self._DataSubSetup, value, 3, 2)
        self.write(self._addrSubSetup, self._DataSubSetup)
    ...
    def setSubSetupBassFreq(self, value, save=1):
        value = self.limit(value, 3)
        self._DataSubSetup = self.setBits(self._DataSubSetup, value, 3, 4)
        self.write(self._addrSubSetup, self._DataSubSetup)
    ...
    def setSubSetupDcMode(self, value, save=1):
        value = self.limit(value, 1)
        self._DataSubSetup = self.setBits(self._DataSubSetup, value, 1, 6)
        self.write(self._addrSubSetup, self._DataSubSetup)
    ...
    def setSubSetupSmooth(self, value, save=1):
        value = self.limit(value, 1)
        self._DataSubSetup = self.setBits(self._DataSubSetup, value, 1, 7)
        self.write(self._addrSubSetup, self._DataSubSetup)
    ...
    def setMixLevel(self, value, save=1):
        value = self.limit(value, 11)
        self._DataMixLevel = self.setBits(self._DataMixLevel, value, 15, 4)
        self.write(self._addrMixLevel, self._DataMixLevel)
    ...
    def setMixLevelLFS(self, value, save=1):
        value = self.limit(value, 1)
        self._DataMixLevel = self.setBits(self._DataMixLevel, value, 1, 0)
        self.write(self._addrMixLevel, self._DataMixLevel)
    ...
    def setMixLevelRFS(self, value, save=1):
        value = self.limit(value, 1)
        self._DataMixLevel = self.setBits(self._DataMixLevel, value, 1, 1)
        self.write(self._addrMixLevel, self._DataMixLevel)
    ...
    def setMixLevelEnable(self, value, save=1):
        value = self.limit(value, 1)
        self._DataMixLevel = self.setBits(self._DataMixLevel, value, 1, 2)
        self.write(self._addrMixLevel, self._DataMixLevel)
    ...
    def setMixLevelSubEnable(self, value, save=1):
        value = self.limit(value, 1)
        self._DataMixLevel = self.setBits(self._DataMixLevel, value, 1, 3)
        self.write(self._addrMixLevel, self._DataMixLevel)
    ...
    def setSpectrumQFactor(self, value, save=1):
        value = self.limit(value, 1)
        self._DataDriverSetup = self.setBits(self._DataDriverSetup, value, 1, 0)
        self.write(self._addrDriverSetup, self._DataDriverSetup)
    ...
    def setSpectrumSource(self, value, save=1):
        value = self.limit(value, 1)
        self._DataDriverSetup = self.setBits(self._DataDriverSetup, value, 1, 2)
        self.write(self._addrDriverSetup, self._DataDriverSetup)
    ...
    def setSpectrumRun(self, value, save=1):
        value = self.limit(value, 1)
        self._DataDriverSetup = self.setBits(self._DataDriverSetup, value, 1, 3)
        self.write(self._addrDriverSetup, self._DataDriverSetup)
    ...
    def setChipReset(self, value, save=1):
        value = self.limit(value, 1)
        self._DataDriverSetup = self.setBits(self._DataDriverSetup, value, 1, 4)
        self.write(self._addrDriverSetup, self._DataDriverSetup)
    ...
    def setChipResetMode(self, value, save=1):
        value = self.limit(value, 1)
        self._DataDriverSetup = self.setBits(self._DataDriverSetup, value, 1, 1)
        self.write(self._addrDriverSetup, self._DataDriverSetup)
    ...
    def setChipClockSource(self, value, save=1):
        value = self.limit(value, 1)
        self._DataDriverSetup = self.setBits(self._DataDriverSetup, value, 1, 5)
        self.write(self._addrDriverSetup, self._DataDriverSetup)
    ...
    def setCouplingMode(self, value, save=1):
        value = self.limit(value, 3)
        self._DataDriverSetup = self.setBits(self._DataDriverSetup, value, 3, 6)
        self.write(self._addrDriverSetup, self._DataDriverSetup)
...
# common op
    def limit(self, value, high, low=0):
        if value > high:
            value = high
        if value < low:
            value = low
    ...
    def setBits(self, data, value, mask, pos):
    ...
    def reset(self):
        ...
        self.setVolumesSoftStep(self._preamp_data.get(self._keyVolumesSoftStep))
        self._reinitVolumes()
        # coupling mode, clock source, reset, spectrum
        # self.write(self._addrDriverSetup, 80)

    # reinit volumes
    def _reinitVolumes(self):
        self.setMainVolume(self._preamp_data.get(self._keyMainVolume), self._VolumesSoftStep)
        self.setLFSpeakerVolume(self._preamp_data.get(self._keyLFSpeaker), self._VolumesSoftStep)
        self.setRFSpeakerVolume(self._preamp_data.get(self._keyRFSpeaker), self._VolumesSoftStep)
        self.setLRSpeakerVolume(self._preamp_data.get(self._keyLRSpeaker), self._VolumesSoftStep)
        self.setRRSpeakerVolume(self._preamp_data.get(self._keyRRSpeaker), self._VolumesSoftStep)
        self.setSubVolume(self._preamp_data.get(self._keySubVolume), self._VolumesSoftStep)
        self.setMixGain(self._preamp_data.get(self._keyMixGain), self._VolumesSoftStep)

    # reset all  values to default
    def resetToDefault(self):
        self._preamp_data.getDefaults()
        self.reset()
